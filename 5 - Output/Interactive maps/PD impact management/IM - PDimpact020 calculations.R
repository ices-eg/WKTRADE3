...
for (imsfd in 1:length(uniMSFD)){
  ...
  idx_stateTresh   <- min(which(hab$avgstate < 0.8 ))
...
areabelow <- subset(Trade_RE1,Trade_RE1$avgstate >0.8 & Trade_RE1$avgsar >0)
belowcsq  <- areabelow$csquares 

areaabove <- subset(Trade_RE1,Trade_RE1$avgstate <0.8 & Trade_RE1$avgsar >0)
