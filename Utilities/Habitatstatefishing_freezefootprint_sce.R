...
# loop each year and calculate state 
  ...
  FisheriesMetfreeze<-cbind(FisheriesMetfreeze, Region@data[match(FisheriesMetfreeze$csquares,Region@data$csquares), c("intercept","slope")])
  
  # depletion rates is based on  Hiddink et al. PNAS 2017 / Rijnsdorp et al. ICES 2020
  for (i in 1: length(Period)){
    loopdata <- FisheriesMetfreeze
 ...
 ### calculate state for TBB_MOL
  ...
  State_reg_freeze <- data.frame(Fisheries$csquares,state_year)


  rm(list= ls()[!(ls() %in% c('pathdir','pathdir_nogit','Assregion','Assunit','Period','AssPeriod','Fisheries',
                              'FisheriesMet','Region','State_reg','EcoReg'))])
